...
module.exports = {
  createStudent: async ({ studentInput }) => {
    try {
      ...
      if (studentInput.password.length < 1)
  ...
  getStudentsWithCoursesName: async ({ name }) => {
    ...
    try {
      ...
      studentsWithCoursesName = students.map(student => ({
        ...
        role: student.role,
        publicId: student.publicId,
        registerCourses: student.registerCourses.map(course => {
  ...
  getStudentCourses: async ({ studentId }) => {
    ...
    try {
      ...
      let filteredCourses = courses.filter(
        course => course.registerStudents.indexOf(studentId) > -1
      );
      if (courses.length === 0) {
